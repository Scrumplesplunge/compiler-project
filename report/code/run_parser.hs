-- Each result is either a success, which means the subtree parsed and parsing
-- can continue, or it is a failure, in which case the subtree did not parse and
-- it may be desirable to display an error message.
...
full_parse p xs = case [full | Success (full, []) <- ps] of
                    [] -> error . show $ failure ps
                    (x:xs) -> x
       where ps = run_parser p xs
