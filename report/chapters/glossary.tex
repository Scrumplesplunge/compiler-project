...
\newglossaryentry{reglang}{
  ...
  description = {
    ...
    regular language is to use a \textit{\gls{regex}}.
  }
}
...
\newglossaryentry{cflang}{
  ...
  description = {
    A context-free language is a language which can be matched by some
    context-free grammer. The collection of context-free languages is a strict
    superset of the collection of \gls{reglang}.
  }
}
...
\newglossaryentry{cfg}{
  ...
  description = {
    Context-free grammars are a way of describing \gls{cflang}s, and are
    made up of collections of rules which describe reductions. The rule
    ...
    suitable for describing programming languages than \gls{reglang}s are.
  }
}
...
\newglossaryentry{lexing}{
  ...
  text = lexing,
  description = {
    The process of converting a stream of characters into a (shorter) stream of
...
\newglossaryentry{transputer}{
  ...
  description = {
    ...
    section \ref{transputer} for a more detailed description.
  }
}
...
\newglossaryentry{vm}{
  ...
  description = {
    The Virtual Machine interprets and executes program bytecode. See section
    \ref{vm} for details.
  }
}
...
\newglossaryentry{lazy-eval}{
  ...
  description = {
    Lazy evaluation is a reduction strategy which only performs work that is
    essential for computing the overall result. This results in a slight
    performance overhead in cases where all intermediate calculations are
    necessary, but can often significantly reduce the computational complexity
    of na\"ive algorithms.
  }
}

\newglossaryentry{static-chain}{
  name = Static Chain,
  text = static chain,
  description = {
    The static chain is the list formed by repeatedly dereferencing the
    \gls{static-link} pointer. It is used to implement closure semantics.
  }
}

\newglossaryentry{static-link}{
  name = Static Link,
  text = static link,
  description = {
    The static link is a pointer which is at a known location in the workspace
    of a process, which points to the location where variables in the enclosing
    scope can be found. This allows the same code to be called within different
    contexts, and is used to implement closure semantics.
  }
}

\newglossaryentry{nagles-algo}{
  name = Nagle's Algorithm,
  text = Nagle's algorithm,
  description = {
    To improve network utilisation, Nagle's algorithm buffers output for a short
    period before sending so that a sequence of small messages can be sent as
    one larger one, thereby reducing the overhead by sending fewer TCP headers.
  }
}
...
\newglossaryentry{combinator}{
  ...
  text = combinator,
  description = {
    A combinator, in the context of formal languages, is a function which
    modifies or combines some number of languages to form a new language. In the
    case of regular expressions, the combinators are functions such as union,
    concatenation, and klein star (zero or more repetitions).
  }
}
...
\newglossaryentry{l-value}{
  ...
  text = l-value,
  description = {
    An l-value is any construct which can be meaningfully assigned to. The term
    "l-value" is short for "left-value", and is borrowed from C nomenclature
    where the assignee appears to the left of the assignment operator.
  }
}
